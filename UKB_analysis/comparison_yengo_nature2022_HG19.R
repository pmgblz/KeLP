######################################################################
  ...
  # Compare the SNPs in Yengo (2022) to the KeLP Rejections #


# This file does the following:
# 1. Read in KelP and Yengo (2022) rejections
# 2. Find matches for KelP rejections by name and location 
# 3. Compare distance of not matched groups to next closet SNP
# 4. Compare knockoff outer node and KelP distances to next closet SNP
#    for those groups that did not have a match
...
height_rejected_eblipr_with_info <- readRDS(paste0(mydir, "farmshare_download/UKB/RDS_eblipr/height_british_unrelatedeblipr_detections_info_gamma0.0125"))

########### ...matching by name #############

# Which snp's can we automatically match by name
height_rejected_eblipr_with_info <- height_rejected_eblipr_with_info %>% 
  mutate(direct_match = ifelse(SNP %in% meta_snps_yengo_nature2022$SNP, 1, 0)) 
sum(height_rejected_eblipr_with_info$direct_match)

# check which groups contain one of the SNPs that could be matched directly
group_contains_direct_match <- height_rejected_eblipr_with_info %>% 
...
height_rejected_eblipr_with_info <- left_join(height_rejected_eblipr_with_info,
                                              group_contains_direct_match, by = "Res_CHR_Group")

# groups that do not contain any snp that can be matched by name
height_rejected_eblipr_with_info %>% 
...
all_groups <- height_rejected_eblipr_with_info %>% pull(Res_CHR_Group) %>% unique()

# filter out all groups that cannot be matched by name 
height_rejections_no_name_match <- height_rejected_eblipr_with_info %>% 
  filter(group_has_at_least_one_direct_match == 0)

# how many groups remain after matching by name?
length(unique(height_rejections_no_name_match$Res_CHR_Group))

# check that there are no inconsistencies 
name_match_snp <- height_rejected_eblipr_with_info %>% filter(direct_match == 1) %>% dplyr::select(SNP, BP.min, BP.max)
...
# for how many could we find a match either by location or by name
height_rejections_name_or_location_match <- height_rejected_eblipr_with_info %>% 
  filter(group_has_at_least_one_direct_match > 0 | Res_CHR_Group %in% corresponding_group$Res_CHR_Group)
length(unique(height_rejections_name_or_location_match$Res_CHR_Group))

# is there overlap between the location match SNPs and the name match SNPs? 
yengo_snps_with_match <- unique(c(corresponding_group$yengo_snp, height_rejected_eblipr_with_info %>% filter(direct_match == 1) %>% pull(SNP) %>% unique()))
length(yengo_snps_with_match)

# for which ones can we not find a match by location and not by name
height_rejections_no_name_no_location_match <- height_rejected_eblipr_with_info %>% 
  filter(group_has_at_least_one_direct_match == 0) %>% 
...
# CHECK DISTANCE TO NEXT SNP in YENGO FOR THOSE GROUPS THAT WE COULD NOT (!!!) MATCH
...
ggsave(paste0(my_out_dir, "figures/", "histogram_bottom95_bpdiff_yengo_hg19.png"), 
...
height_rejections_name_location_match <- height_rejected_eblipr_with_info %>% 
  filter(group_has_at_least_one_direct_match > 0 | Res_CHR_Group %in% corresponding_group$Res_CHR_Group) 
...
# CHECK DISTANCE TO NEXT SNP in YENGO FOR THOSE GROUPS THAT WE COULD (!!!) MATCH
...
ggsave(paste0(my_out_dir, "figures/", "histogram_bottom95_bpdiff_matched_next_yengo_hg19.png"), 
...
ggsave(paste0(my_out_dir, "figures/", "density_bottom95_bpdiff_next_yengo_hg19.png"), 
       plot = density_bottom95_bpdiff_next_yengo, device = png, width = 7, height = 5)

# save snps
next_snps %>% 
  mutate(Method = "Eblipr") -> next_snps 

saveRDS(next_snps, paste0(my_out_dir, "RDS/next_snps_eblipr"))


# compare to knockoff outer nodes rejections for those that we could not match
next_snps_knockoff <- readRDS(paste0(my_out_dir, "RDS/next_snps_knockoff"))
next_snps_knockoff %>% 
  mutate(Method = "Knockoff") -> next_snps_knockoff

next_snps_eblipr_no_match <- next_snps %>% 
  filter(Matched == "No")

next_snps_knockoff_no_match <- next_snps_knockoff %>% 
  filter(Matched == "No")

next_snps_knockoff_eblipr_no_match <- rbind(next_snps_eblipr_no_match, next_snps_knockoff_no_match)

# density comparison of eblipr and knockoff outer nodes for groups that did not have a match
next_snps_knockoff_eblipr_no_match  %>% 
  filter(bp_diff <= quantile(bp_diff, 0.95)) %>% 
  ggplot(aes(x = bp_diff, fill = Method, colour = Method)) + 
  geom_density(alpha = 0.5, position = "identity")  + 
  theme_minimal() + labs(x = "Difference in BP", y = "Density") + 
  scale_fill_manual(values=c("#E69F00", "#56B4E9")) + 
  scale_color_manual(values=c("#E69F00", "#56B4E9")) -> density_bottom95_bpdiff_eblipr_knockoff

ggsave(paste0(my_out_dir, "figures/", "density_bottom95_bpdiff_eblipr_knockoff.png"), 
       plot = density_bottom95_bpdiff_eblipr_knockoff, device = png, width = 7, height = 5)
